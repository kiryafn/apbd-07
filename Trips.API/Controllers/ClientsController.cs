...
public class ClientsController : ControllerBase
{
    ...
    [HttpDelete("{userId:int}/tours/{tripId:int}")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> DeleteUserTrip([FromRoute] int userId, [FromRoute] int tripId, CancellationToken token = default)
    {
        if (userId <= 0 || tripId <= 0)
            return BadRequest();

        var success = await _clientService.DeleteTourByIdAsync(userId, tripId, token);
        if (!success)
            return NotFound();
