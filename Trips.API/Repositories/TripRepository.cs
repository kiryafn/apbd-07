...
public class TripRepository : ITripRepository
{
    ...
    public async Task<bool> ClientTripExistsAsync(int clientId, int tripId, CancellationToken token = default)
    {
    ...
    {
        if (!await ClientTripExistsAsync(clientId, tripId, token)) return false;
        ...
        try
        {
            ...
            await using var deleteCmd = new SqlCommand(deleteQuery, con, (SqlTransaction)transaction);
